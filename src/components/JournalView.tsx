...
export default function JournalView({ selectedArticleId, setSelectedArticleId }: JournalViewProps) {
  ...
  if (selectedArticleId) {
    ...
    if (article) {
      return (
        <article className="min-h-screen bg-[#121414] text-[#e2e2e2] pt-28 pb-24 px-6">
          <div className="max-w-3xl mx-auto space-y-8">
            {/* Back Button */}
            <button
              onClick={() => setSelectedArticleId(null)}
              className="flex items-center gap-2 text-[#edc14d] hover:text-white font-sans text-xs tracking-widest font-semibold uppercase group cursor-pointer"
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <span className="flex items-center gap-1.5 text-white/50 text-[11px] uppercase tracking-wider font-sans">
                  <Clock size={12} /> {article.readTime}
                </span>
                <span className="text-white/30 hidden sm:inline">•</span>
                <span className="text-white/50 text-[11px] uppercase tracking-wider font-sans hidden sm:inline">
                  {article.date}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between border-y border-white/5 py-4 mt-6">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#1A4233] border border-white/10 flex items-center justify-center text-[#edc14d] font-bold font-serif text-sm">
                    {article.author.charAt(0)}
                  </div>
                  <div>
                    <p className="text-xs font-sans tracking-wider text-white font-semibold uppercase">{article.author}</p>
                    <p className="text-[10px] text-white/40 font-sans uppercase">Plantsource Wholesale Editorial Board</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="p-2 rounded-full hover:bg-white/5 text-[#edc14d] cursor-pointer"
                  >
                    <Bookmark size={18} className={bookmarkedIds.includes(article.id) ? 'fill-current' : ''} />
                  </button>
                  <button 
                    onClick={(e) => handleShare(article.id, e)}
                    className="p-2 rounded-full hover:bg-white/5 text-[#edc14d] relative cursor-pointer"
                  >
                    <AnimatePresence>
            ...
            {/* Main Featured Image */}
            <div className="w-full h-[320px] md:h-[480px] rounded-3xl overflow-hidden border border-white/5">
              <img src={article.image} alt={article.title} className="w-full h-full object-cover" />
            </div>

            {/* Article Body Content */}
            <div className="font-sans text-white/80 space-y-6 leading-relaxed text-base md:text-lg font-light pt-4 border-b border-white/5 pb-10">
              {article.content.split('\n\n').map((paragraph, idx) => {
                if (paragraph.startsWith('### ')) {
                  return (
                    <h3 key={idx} className="font-serif text-2xl text-white font-semibold pt-6 pb-2">
                      {paragraph.replace('### ', '')}
                    </h3>
                  );
                }
                if (paragraph.startsWith('1. ') || paragraph.startsWith('- ')) {
                  ...
                  return (
                    <ul key={idx} className="space-y-3 pl-5 list-disc text-white/70">
                      {items.map((it, i) => {
            ...
            <div className="bg-[#1A4233]/20 border border-white/5 rounded-3xl p-8 text-center space-y-4">
              <Sparkles className="text-[#edc14d] mx-auto" size={24} />
              <h4 className="font-serif text-xl font-medium text-white">Nourished with these ideas?</h4>
              <p className="text-xs text-[#c1c8c2] font-light max-w-sm mx-auto">
                Subscribe to receive our meticulously researched bio-nutrient reports and gourmet secret organic recipes.
              </p>
              <form onSubmit={handleNewsletterSubmit} className="flex max-w-md mx-auto items-center border-b border-white/20 pb-2 pt-2 focus-within:border-[#edc14d] transition-transform">
                <input
                  type="email"
                  value={newsletterEmail}
                  onChange={(e) => setNewsletterEmail(e.target.value)}
                  placeholder="Your Email Address"
                  className="bg-transparent border-none text-xs w-full focus:outline-none p-0 focus:ring-0 text-white placeholder-white/30"
                />
              ...
              {newsletterSuccess && (
                <p className="text-emerald-400 text-xs font-sans">You have entered the Inner Circle.</p>
              )}
            </div>
          </div>
        </article>
  ...
  return (
    <div className="min-h-screen bg-[#121414] text-[#e2e2e2] pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* Categories/Tgas Filter (Image 2) */}
        <div className="flex flex-wrap items-center justify-center gap-3 mb-16 border-b border-white/5 pb-8" id="journal-filter-trigger">
          {[
        ...
        <div className="space-y-16">
          ...
          {activeTag === 'all' && (
            <div 
              ...
              className="group bg-[#1A4233]/20 border border-white/5 rounded-3xl overflow-hidden cursor-pointer flex flex-col lg:flex-row transition-all duration-300 hover:bg-[#1a4233]/40"
            >
              {/* Image side */}
              ...
              <div className="p-8 md:p-12 lg:w-1/2 flex flex-col justify-center space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white/40 font-sans text-[11px] uppercase tracking-wider">
                    12 MIN READ
                  </span>
                </div>
                
                <h2 className="font-serif text-3xl md:text-4xl text-white font-medium hover:text-[#edc14d] transition-colors leading-tight">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                  ...
                  <span className="text-white/30 font-sans text-xs font-light uppercase">
                    MAY 28, 2026
                  </span>
                </div>
              </div>
            ...
            {filteredArticles.filter(art => art.id !== 'art_1').map((article) => {
              ...
              return (
                <div 
                  ...
                  className="group bg-[#1A4233]/15 hover:bg-[#1a4233]/30 border border-white/5 rounded-3xl overflow-hidden cursor-pointer flex flex-col justify-between transition-all duration-300"
                >
                  <div className="space-y-6">
                    ...
                    <div className="px-8 space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-white/40 font-sans text-[10px] uppercase font-light">
                          {article.readTime}
                        </span>
                      </div>

                      <h3 className="font-serif text-2xl font-medium text-white group-hover:text-[#edc14d] transition-colors leading-snug">
                        {article.title}
                      </h3>

                      <p className="font-sans text-xs text-[#c1c8c2] line-clamp-3 leading-relaxed font-light">
                        {article.excerpt}
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Footer card action */}
                  <div className="px-8 pb-8 pt-6 flex items-center justify-between border-t border-white/5 mt-6">
                    {article.category === 'recipes' ? (
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-full bg-[#1A4233] flex items-center justify-center text-xs">🌱</div>
                        <span className="text-white/40 font-sans text-[10px] tracking-wider uppercase font-semibold">
                    ...
                    <span className="text-white/30 font-sans text-[10px] tracking-wider uppercase">
                      {article.date}
                    </span>
                  </div>
                </div>
        ...
        <section className="mt-24 bg-[#013120] border border-white/5 text-center p-12 md:p-20 rounded-3xl relative overflow-hidden max-w-4xl mx-auto">
          ...
          <div className="relative z-10 max-w-md mx-auto space-y-6">
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-4 pt-4">
              <input 
                ...
                maxLength={80}
                required
                className="w-full bg-[#121414] border border-white/10 rounded-full py-4 px-6 text-center text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#edc14d] focus:ring-1 focus:ring-[#edc14d] transition-all"
              />
              <button 
                type="submit" 
